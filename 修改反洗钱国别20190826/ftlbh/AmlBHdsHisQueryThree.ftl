...
<@CommonQueryMacro.page title="大额交易查询">
		<@CommonQueryMacro.CommonQuery id="AmlBHdsHisQueryThree" init="false" submitMode="all" navigate="false" >
	...
	var sysTxdate = ${statics["com.huateng.ebank.business.common.GlobalInfo"].getCurrentInstanceWithoutException().getTxdate()?string("yyyyMMdd")};   
		...
		function initCallGetter_post() {
			AmlBHdsHisQueryThree_interface_dataset.setValue("qworkDateStart",sysTxdate);
			AmlBHdsHisQueryThree_interface_dataset.setValue("qworkDateEnd",sysTxdate);
			AmlBHdsHisQueryThree_interface_dataset.setValue("qrecStatus","06");
		}
		...
		function doDetail(id){
			showWin("大额交易信息管理明细","/fpages/hfaml3/ftl/bh/AmlBHdsHisAddThree.ftl?id=" + id + "&op=detail","window","flushPage()",window);
		}
		
		
		//确认提交
		function btSubmit_onClickCheck(button){
			var record = AmlBHdsHisQueryThree_dataset.firstUnit;
			var chk=0;
			while(record){
	    ...
	    function btSubmit_postSubmit(button){
	    	...
	    	AmlBHdsHisQueryThree_dataset.flushData(AmlBHdsHisQueryThree_dataset.pageIndex);
	    }
