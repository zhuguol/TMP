...
<@CommonQueryMacro.page title="大额无规则交易查询">
		<@CommonQueryMacro.CommonQuery id="AmlTradInfoNorules" init="false" submitMode="all" navigate="false" >
	...
	function initCallGetter_post() {
		AmlTradInfoNorules_interface_dataset.setValue("qetlDate",sysTxdate);
	}
		
	function btDownload_onClick(){
		window.document.getElementById("btDownload").disabled = true;
		createTimerIfNull();
		var qetlDate = formatDate(AmlTradInfoNorules_interface_dataset.getValue("qetlDate"));
		var qcustNo = AmlTradInfoNorules_interface_dataset.getValue("qcustNo");
		var qbussFlag = AmlTradInfoNorules_interface_dataset.getValue("qbussFlag");
		window.location.href = "${contextPath}/filedownload/AmlTradInfoNoRulesDownoloadAction.do?qbussFlag="+qbussFlag+"&qcustNo="+qcustNo+"&qetlDate="+qetlDate;
	}
