...
<table align="left">
    <tr>
        <td>
            <@CommonQueryMacro.CommonQuery id="AmlIBSDsColl2" init="false" submitMode="current">
            <table width="100%">
                <tr>
...
<script language="JavaScript">
	...
	function initCallGetter_post() {
		AmlIBSDsColl2_interface_dataset.setValue("qworkDateStart",sysTxdate);
		AmlIBSDsColl2_interface_dataset.setValue("qworkDateEnd",sysTxdate);
	}
	//初始化详细链接
	...
	function doDetail(id,reportId,workDate) {
		showWin("可疑补正报文详细","/fpages/hf/ftl/AmlBSDsCollInfo2.ftl?op=detail&reportId="+reportId+"&id="+id+"&workDate="+workDate,"window","flushPage()",window);
	}
	//补录
	function btMod_onClickCheck(button){
		var record = AmlIBSDsColl2_dataset.firstUnit;
		var chk=0;
		var id;
		var reportId;
		var workDate;
		...
		showWin("可疑补正报文补录","/fpages/hf/ftl/AmlBSDsCollInfo2.ftl?op=update&reportId="+reportId+"&id="+id+"&workDate="+formatDate(workDate),"window","flushPage()",window);
		}
	}
   	...
   	function updateReport(ids,flag){
		AMLVaildService.updateBSReport(ids,flag,function(mgs){
			alert(mgs);
			AmlIBSDsColl2_dataset.flushData(AmlIBSDsColl2_dataset.pageIndex);
		});
	}
