...
<@CommonQueryMacro.page title="可疑纠错报文补录">
<table align="left">
    <tr>
        <td>
            <@CommonQueryMacro.CommonQuery id="AmlCBSDsColl2" init="false" submitMode="current">
            <table width="100%">
                <tr>
                ...
                <tr>
                    <td colspan="2">
                    <@CommonQueryMacro.DataTable id="datatable" paginationbar="btMod,-,btDel,-,btUnReport" fieldStr="select,oper,workDate,reportId,recStatus,reportType[120],brno,finm[200],firc[250],fict[200],ficd[200],stcr[200],ssdg[200],lstUpdTlr,lstUpdTm"  width="100%"  height="480"  hasFrame="true" readonly="true" />
                    </td>
                </tr>
            </table>
...
<script language="JavaScript">
	...
	function initCallGetter_post() {
		AmlCBSDsColl2_interface_dataset.setValue("qworkDateStart",sysTxdate);
		AmlCBSDsColl2_interface_dataset.setValue("qworkDateEnd",sysTxdate);
	}
	//初始化详细链接
	...
	//纠错
	function btJiucuo_onClick(button){
	   var id = guid();
	   var maxBatchNo = 0;
	   var reportId  = 0;
	   PrivAction.getMaxBatchNo(function(HbatchNo){
		...
		showWin("可疑报文纠错","/fpages/hf/ftl/AmlCBSDsCollInfo2.ftl?op=new&reportId="+reportId+"&id="+id+"&workDate="+sysTxdate,"window","flushPage()",window);
	   });
	    return true;
	}
	...
	function doDetail(id,reportId,workDate) {
		showWin("可疑纠错报文详细","/fpages/hf/ftl/AmlCBSDsCollInfo2.ftl?op=detail&reportId="+reportId+"&id="+id+"&workDate="+workDate,"window","flushPage()",window);
	}
	
	//补录
	function btMod_onClickCheck(button){
		var record = AmlCBSDsColl2_dataset.firstUnit;
		var chk=0;
		var id;
		var reportId;
		var workDate;
	...
	function doDelete(id,workDate){
    	...
    	AmlCBSDsColl2_dataset.flushData(AmlCBSDsColl2_dataset.pageIndex);
	}
   ...
   function btUnReport_onClickCheck(button){
	...
	//刷新数据
	function flushPage(){
		AmlCBSDsColl2_dataset.flushData(1);
	}
