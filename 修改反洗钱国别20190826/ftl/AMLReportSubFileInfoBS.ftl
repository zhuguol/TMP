...
<script type='text/javascript' src='${contextPath}/dwr/interface/AMLReportFile.js'> </script>
		<@CommonQueryMacro.CommonQuery id="AMLReportSubFileInfoBS" init="false" submitMode="current" >
...
<script language="JavaScript">
	...
	function initCallGetter_post() {
		AMLReportSubFileInfoBS_interface_dataset.setValue("qcurrentfile",'BS');
	}
	...
	function backFile(filePack){
		if(window.confirm('确定要将文件：['+filePack+'.zip]进行回退操作吗？')){
			AMLReportFile.delGenAMLFileBs(filePack,{
				callback:execDelGenAMLFileCallback,
				errorHandler:function(msg,exc){
				AMLReportSubFileInfoBS_dataset.flushData();
