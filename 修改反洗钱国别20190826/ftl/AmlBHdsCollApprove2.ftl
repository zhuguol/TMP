...
<@CommonQueryMacro.page title="大额普通报文不上报审核">
		<@CommonQueryMacro.CommonQuery id="AmlBHdsCollApprove2" init="false" submitMode="all" navigate="false" >
	...
	var sysTxdate = ${statics["com.huateng.ebank.business.common.GlobalInfo"].getCurrentInstanceWithoutException().getTxdate()?string("yyyyMMdd")};   
		...
		function initCallGetter_post() {
			AmlBHdsCollApprove2_interface_dataset.setValue("qworkDateStart",sysTxdate);
			AmlBHdsCollApprove2_interface_dataset.setValue("qworkDateEnd",sysTxdate);
			AmlBHdsCollApprove2_interface_dataset.setValue("qrecStatus","00");
		}
		...
		function doDetail(id){
			showWin("大额普通报文明细","/fpages/hf/ftl/AmlBHdsCollAdd2.ftl?id=" + id + "&op=detail"+ "&opFromAmlBHdsCollApprove2=flag","window","flushPage()",window);
		}
		...
		function btApprove_onClickCheck(button){
			...
			var hasAudit = false;
			var record = AmlBHdsCollApprove2_dataset.getFirstRecord();
			var recIds = "";
			while(record){
	   	...
	   	function approveUNReport(ids,approveStatusChoose,approveResultChoose){
			...
			alert(mgs);
			AmlBHdsCollApprove2_dataset.flushData(AmlBHdsCollApprove2_dataset.pageIndex);
			});
		}
		...
		function btUNApprove_onClick(){
	   	...
	   	function btSubmit_onClickCheck() {
			...
			while(record){
		   	...
		   	var approveResultChoose = AmlBHdsCollApprove2_dataset.getValue("approveResultChoose");
