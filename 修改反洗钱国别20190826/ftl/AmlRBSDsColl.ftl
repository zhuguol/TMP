...
<table align="left">
    <tr>
        <td>
            <@CommonQueryMacro.CommonQuery id="AmlRBSDsColl" init="false" submitMode="current">
            <table width="100%">
                <tr>
...
<script language="JavaScript">
	...
	function doDetail(id,reportId,workDate) {
		showWin("可疑重发报文补录","/fpages/hf/ftl/AmlBSDsCollInfo.ftl?op=detail&reportId="+reportId+"&id="+id+"&workDate="+workDate,"window","flushPage()",window);
	}
	//修改
	function btMod_onClickCheck(button){
		var record = AmlRBSDsColl_dataset.firstUnit;
		var chk=0;
		...
		showWin("可疑重发报文补录","/fpages/hf/ftl/AmlBSDsCollInfo.ftl?op=update&reportId="+reportId+"&id="+id+"&workDate="+formatDate(workDate),"window","flushPage()",window);
		}
	}
	...
	//上报
   	...
   	function updateReport(ids,flag){
		AMLVaildService.updateBSReport(ids,flag,function(mgs){
			alert(mgs);
			AmlRBSDsColl_dataset.flushData(AmlRBSDsColl_dataset.pageIndex);
		});
	}
	//刷新数据
	function flushPage(){
		AmlRBSDsColl_dataset.flushData(1);
	}
