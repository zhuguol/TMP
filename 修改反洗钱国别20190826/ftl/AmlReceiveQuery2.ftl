...
<@CommonQueryMacro.page title="大额申领查询">
		<@CommonQueryMacro.CommonQuery id="AmlReceiveQuery2" init="false" submitMode="all" navigate="false" >
	...
	<script language="JavaScript">
		...
		function initCallGetter_post() {
			AmlReceiveQuery2_interface_dataset.setValue("qetlDate",sysTxdate);
		}
			
		function btDownload_onClick(){
			window.document.getElementById("btDownload").disabled = true;
			createTimerIfNull();
			var qetlDate = formatDate(AmlReceiveQuery2_interface_dataset.getValue("qetlDate"));
			var qcustNo = AmlReceiveQuery2_interface_dataset.getValue("qcustNo");
			var qbussFlag = AmlReceiveQuery2_interface_dataset.getValue("qbussFlag");
			var sourceFlag = 2;
