...
<@CommonQueryMacro.page title="审核申领">
<@CommonQueryMacro.CommonQuery id="CreditCardApproveApplyInfo" init="false" submitMode="all" navigate="false" >
...
function btShenlin_onClickCheck(button){
	...
	var hasAudit = false;
	var record = CreditCardApproveApplyInfo_dataset.getFirstRecord();
	var ids = "";
	while(record){
   	...
   	if (!hasSelected) {
   		...
   		ids = ids.substring(1);
   		CreditCardApproveApplyInfo_dataset.setParameter("ids",ids);
   	}
   	return true;
}
...
function btShenlin_postSubmit(button) {
	...
	createTimer(2000);
	CreditCardApproveApplyInfo_dataset.flushData();
}

//定位一条记录
function locate(id) {
    var record = CreditCardApproveApplyInfo_dataset.find(["id"], [id]);
    if (record) {
        CreditCardApproveApplyInfo_dataset.setRecord(record);
    }
}

function flushPage() {
    CreditCardApproveApplyInfo_dataset.flushData(CreditCardApproveApplyInfo_dataset.pageIndex);
}
