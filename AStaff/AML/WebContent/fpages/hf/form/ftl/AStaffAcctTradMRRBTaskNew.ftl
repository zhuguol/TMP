...
<@CommonQueryMacro.page title="员工交易监测月报员工管理">
	<@CommonQueryMacro.CommonQuery id="AStaffAcctTradMRRBTaskNew" init="false" submitMode="all" navigate="false" >
...
<script language="JavaScript">
	...
	function initCallGetter_post() {
		//交易起始日期
		AStaffAcctTradMRRBTaskNew_interface_dataset.setValue("etlDateStart",sysTxdate);
		//交易结束日期
		AStaffAcctTradMRRBTaskNew_interface_dataset.setValue("etlDateEnd",sysTxdate);
		//员工编号
		AStaffAcctTradMRRBTaskNew_interface_dataset.setValue("staffNumbers","01");
	}
	...
	function btnSelect_onClickCheck(button){
		var record = AStaffAcctTradMRRBTaskNew_dataset.firstUnit;
		var updateCount = 0;
		var alertIds;
		while(record){
		...
		showWin("告警信息、客户信息、调查信息和质检信息","/fpages/hf/form/ftl/AStaffAcctTradMRRBUpdate.ftl?alertIds="+alertIds+"&op=detail","window","flushPage()",window);
	}
	...
	function btnDownLoad_onClick(){
		...
		var aStaffAcctTradMRRB = AStaffAcctTradMRRBTaskNew_dataset.getFirstRecord();
		...
		var tableName = "AStaffAcctTradMRRBTaskNew";
	...
	function getExportStatus(){
		PrivAction.getExportFlag("AStaffAcctTradMRRBTaskNew",function(data){
			exportCallBack(data);
		});
	}
	...
	function btnReceive_onClickCheck(button){
		...
		while(record){
			...
			if(temp){
				...
				if(status == "03"){//调查阶段
					alert("告警号：'"+ alertIds +"'的记录调查阶段为：'03-Closed by FCTM'不可以进行分派！");
					return false;
				}else if(status == "04"){
					alert("告警号：'"+ alertIds +"'的记录调查阶段为：'04-Closed by President'不可以进行分派！");
					return false;
				}else if(status == "05"){
					alert("告警号：'"+ alertIds +"'的记录调查阶段为：'05-Closed by VPHOST'不可以进行分派！");
					return false;
				}
				updateCount ++;
			}
	...
	function btnConfirm_onClickCheck(){
		var selectTlrno = AStaffAcctTradMRRBTaskNew_dataset.getValue("selectTlrno");
		...
		AStaffAcctTradMRRBTaskNew_dataset.setParameter("selectTlrno",selectTlrno);
		return true;
	}
	...
	//刷新数据
	function flushPage(){
		AStaffAcctTradMRRBTaskNew_dataset.flushData(AStaffAcctTradMRRBTaskNew_dataset.pageIndex);
	}
</script>
