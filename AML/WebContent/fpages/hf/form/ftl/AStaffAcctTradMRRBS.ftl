...
<@CommonQueryMacro.page title="员工交易监测月报">
	<@CommonQueryMacro.CommonQuery id="AStaffAcctTradMRRBS" init="false" submitMode="allchange" navigate="false" >
...
<script language="JavaScript">
	...
	function initCallGetter_post() {
		//交易起始日期
		AStaffAcctTradMRRBS_interface_dataset.setValue("etlDateStart",sysTxdate);
		//交易结束日期
		AStaffAcctTradMRRBS_interface_dataset.setValue("etlDateEnd",sysTxdate);
		//员工编号
		AStaffAcctTradMRRBS_interface_dataset.setValue("staffNumbers","01");
	}
	...
	function btnSelect_onClickCheck(button){
		var record = AStaffAcctTradMRRBS_dataset.firstUnit;
		var updateCount = 0;
		var alertIds;
		while(record){
		...
		if(updateCount == 0){
			...
			if(staffNumber == ""){
				...
				if(approveStatus == "01"){
					...
					return false;
				}else if(approveStatus != "00" && recordUpdTlr !=""){
			    	showWin("告警信息、客户信息和调查信息","/fpages/hf/form/ftl/AStaffAcctTradMRRBUpdate.ftl?alertIds="+alertIds+"&op=make","window","flushPage()",window);
				}else{
					alert("当前记录还未进行调查！");
					return false;
				}
	...
	function btnSubmit_onClickCheck(){
		var checkDateChoose = AStaffAcctTradMRRBS_dataset.getValue("checkDateChoose");
		if(checkDateChoose.length == ''){
			alert("请选择初步调查日期！");
		   	return false;
		}
	    AStaffAcctTradMRRBS_dataset.setParameter("checkDateChoose",checkDateChoose);
	...
	//刷新数据
	function flushPage(){
		AStaffAcctTradMRRBS_dataset.flushData(AStaffAcctTradMRRBS_dataset.pageIndex);
	}
</script>
