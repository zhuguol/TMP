...
<script language="JavaScript">
	...
	function btnDownLoad_onClick(){
		if(confirm("确认下载数据吗?")){
			...
			//20200824 add
			//上报起始日期
			var reportDateStart = AStaffAcctTrad_interface_dataset.getString("reportDateStart");
			//上报结束日期
			var reportDateEnd = AStaffAcctTrad_interface_dataset.getString("reportDateEnd");
			//(初步调查)上报的可疑类型
			var potentialCaseTypes = AStaffAcctTrad_interface_dataset.getString("potentialCaseTypes");
			//(收到反馈)判定的可疑类型
			var concludedTypes = AStaffAcctTrad_interface_dataset.getString("concludedTypes");
			//下载模板名
