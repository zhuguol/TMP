...
<table align="left" id="CustSearchResultP" width="100%">
	...
	<tr>
		<td colspan="2">
			<@CommonQueryMacro.DataTable id ="datatable1" paginationbar="btnDownLoadP" fieldStr="uploadDate[120],batchNo[150],custNo[150],custName[250]" width="100%" height="480" hasFrame="true" readonly="true"/>
  		</td>
	</tr>
</table>
...
<table align="left" id="CustSearchResultC" width="100%">
	...
	<tr>
		<td colspan="2">
			<@CommonQueryMacro.DataTable id ="datatable3" paginationbar="btnDownLoadC" fieldStr="uploadDate[120],batchNo[150],custNo[150],custName[250]" width="100%" height="480" hasFrame="true" readonly="true"/>
  		</td>
	</tr>
</table>
</@CommonQueryMacro.CommonQuery>
<script language="JavaScript">
	...
	function btnDownLoadP_onClick(){
		if(confirm("确认下载数据吗?")){
			...
			if(qDateStartP == '' || !qDateStartP || qDateStartP.length == 0){
				alert('请选择"开始日期"！');
				return false;
			}
			if(qDateEndP == '' || !qDateEndP || qDateEndP.length == 0){
				alert('请选择"结束日期"！');
				return false;
			}
			if(qTypeDataP == '' || !qTypeDataP || qTypeDataP.length == 0){
				alert('请选择"检索结果类型"！');
				return false;
			}
			if(qBatchNoP == '' || !qBatchNoP || qBatchNoP.length == 0){
				qBatchNoP = "";
			}
	...
	function btnDownLoadL_onClick(){
		if(confirm("确认下载数据吗?")){
			...
			if(qDateStartL == '' || !qDateStartL || qDateStartL.length == 0){
				alert('请选择"开始日期"！');
				return false;
			}
			if(qDateEndL == '' || !qDateEndL || qDateEndL.length == 0){
				alert('请选择"结束日期"！');
				return false;
			}
			if(qTypeDataL == '' || !qTypeDataL || qTypeDataL.length == 0){
				alert('请选择"检索结果类型"！');
				return false;
			}
			if(qBatchNoL == '' || !qBatchNoL || qBatchNoL.length == 0){
				qBatchNoL = "";
			}
	...
	function btnDownLoadC_onClick(){
		if(confirm("确认下载数据吗?")){
			...
			if(qDateStartC == '' || !qDateStartC || qDateStartC.length == 0){
				alert('请选择"开始日期"！');
				return false;
			}
			if(qDateEndC == '' || !qDateEndC || qDateEndC.length == 0){
				alert('请选择"结束日期"！');
				return false;
			}
			if(qTypeDataC == '' || !qTypeDataC || qTypeDataC.length == 0){
				alert('请选择"检索结果类型"！');
				return false;
			}
			if(qBatchNoC == '' || !qBatchNoC || qBatchNoC.length == 0){
				qBatchNoC = "";
			}
