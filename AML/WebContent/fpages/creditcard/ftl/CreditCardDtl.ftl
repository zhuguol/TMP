...
function amplifyfile(proofId){
	...
	var url = encodeURI(encodeURI("/fpages/creditcard/jsp/CreditCardProofMould.jsp?proofId="+proofId));
	currentSubWin = openSubWin("pageWinId", "凭证信息", url ,"800","720");
}
...
function btDelete_onClick(button){
	...
	while(record){
		...
		if( v_selected == true ){
			...
			if(status == '2'){
				easyMsg.warn("其中有记录状态为'2-短信文件已生成'的短信信息，不可以删除！");
				return false;
			}
			ids = ids + "," + id;
		}
