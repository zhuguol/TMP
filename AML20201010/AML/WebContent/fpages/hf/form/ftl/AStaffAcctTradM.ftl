...
<script language="JavaScript">
	...
	function btnDownLoad_onClick(){
		if(confirm("确认下载数据吗?")){
			...
			//记录状态
			var approveStatuss = AStaffAcctTradM_interface_dataset.getString("approveStatuss");
			//20200824 add
			//上报起始日期
			var reportDateStart = AStaffAcctTradM_interface_dataset.getString("reportDateStart");
			//上报结束日期
			var reportDateEnd = AStaffAcctTradM_interface_dataset.getString("reportDateEnd");
			//(初步调查)上报的可疑类型
			var potentialCaseTypes = AStaffAcctTradM_interface_dataset.getString("potentialCaseTypes");
			//(收到反馈)判定的可疑类型
			var concludedTypes = AStaffAcctTradM_interface_dataset.getString("concludedTypes");
			//下载模板名
	...
	function btnUpload_onClick(){
		currentSubWin = openSubWin("pageWinId", "员工交易监测月报", "/fpages/hf/jsp/uploadAStaffAcctTrad.jsp?fileflag=AStaffAcctTradM","600","200");
	}
