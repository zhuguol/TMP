...
<script language="JavaScript">
	...
	function btnDownLoad_onClick(){
		if(confirm("确认下载数据吗?")){
			...
			//记录状态
			var approveStatuss = AStaffAcctTradMRRB_interface_dataset.getString("approveStatuss");
			//20200824 add
			//上报起始日期
			var reportDateStart = AStaffAcctTradMRRB_interface_dataset.getString("reportDateStart");
			//上报结束日期
			var reportDateEnd = AStaffAcctTradMRRB_interface_dataset.getString("reportDateEnd");
			//(初步调查)上报的可疑类型
			var potentialCaseTypes = AStaffAcctTradMRRB_interface_dataset.getString("potentialCaseTypes");
			//(收到反馈)判定的可疑类型
			var concludedTypes = AStaffAcctTradMRRB_interface_dataset.getString("concludedTypes");
			if(etlDateStart == ''){
				alert("请选择交易起始日期！");
				return false;
			}
