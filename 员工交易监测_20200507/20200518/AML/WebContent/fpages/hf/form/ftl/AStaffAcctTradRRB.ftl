		...
		if(confirm("确认下载数据吗?")){
			...
			while(aStaffAcctTradRRB){
				...
				if(selected == true){
					...
					if(approveStatus == "00" || tlrno == recordUpdTlr){
						if(status == "03"){
							alert("告警号：'"+ alertId +"'的记录调查阶段为：'03-Closed by FCTM'不可以下载！");
							arraySelected = null;
							return false;
						}else if(status == "04"){
							alert("告警号：'"+ alertId +"'的记录调查阶段为：'04-Closed by President'不可以下载！");
							arraySelected = null;
							return false;
						}else if(status == "05"){
							alert("告警号：'"+ alertId +"'的记录调查阶段为：'05-Closed by VPHOST'不可以下载！");
							arraySelected = null;
							return false;
						}else{
			...
			if(!flag){
				if(statuss == "03"){
					alert("调查阶段为：'03-Closed by FCTM'的记录不可以下载！");
					return false;
				}else if(statuss == "04"){
					alert("请查看告警号为：'04-Closed by President'的记录不可以下载！");
					return false;
				}else if(statuss == "05"){
					alert("请查看告警号为：'05-Closed by VPHOST'的记录不可以下载！");
					return false;
				}
			}
			//告警号
