...
<@CommonQueryMacro.CommonQuery id="AStaffAcctTradRRBUpdate" init="true" insertOnEmpty="true" submitMode="allchange" navigate="false">
				   ...
				   <td width="25%">
				   		<@CommonQueryMacro.GroupBox id="groupbox1" label="调查信息" expand="true">
							<table frame="void" class="grouptable" width="100%">
								...
								<tr>  
									<td width="25%" width="25%" colspan="1" align="right" nowrap class="labeltd">反馈结果(从FCTM或者President指定部门)</td>
...
<script language="javascript">
	...
	function initCallGetter_post(){
  		//详细页面设置只读、其他按钮全部屏蔽
  		if(op == "detail"){
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("dataDate",true);//数据日期
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("alertId",true);//告警号
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("ruleCode",true);//规则代码
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("branCode",true);//机构号
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("custNo",true);//客户号
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("custName",true);//客户名称/姓名
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("custIdCertNo",true);//客户身份证件号码
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("ctnt",true);//客户国籍
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("ctar",true);//客户住址/经营地址
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("staffNumber",true);//员工编号
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("staffDepartment",true);//员工所在部门
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("staffJobTitle",true);//员工职位名称
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("reportDOrM",true);//日报/月报
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("appearanceTime",true);//(日报当月/月报当季)被抓出次数
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("checkDate",true);//初步调查日期
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("investigator",true);//调查者
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("status",true);//调查阶段
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("managerName",true);//直线经理姓名
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("invesResultByOps",true);//初步调查结论(by OPS)
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("caseId",true);//上报案件编号
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("reportDate",true);//上报日期
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("potentialCaseType",true);//(初步调查)上报的可疑类型
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("feedbackDate",true);//收到反馈时间
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("concludedType",true);//(收到反馈)判定的可疑类型
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("feedbackFrom",true);//反馈结果(从FCTM或者GB/GF指定部门)
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("comfirmedSusp",true);//是否判定可疑(及由哪方判定)
  			AStaffAcctTradRRBUpdate_dataset.setFieldReadOnly("alterUar",true);//反馈案件编号
  			//将保存按钮隐藏
