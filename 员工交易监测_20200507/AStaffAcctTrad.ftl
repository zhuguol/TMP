...
<script language="JavaScript">
			...
			if(staffNumber == ""){
				...
				if(approveStatus == "03"){
					...
					if(approveStatus == "00" || tlrno == recordUpdTlr){
			    		showWin("告警信息、客户信息和调查信息","/fpages/hf/form/ftl/AStaffAcctTradUpdate.ftl?alertIds="+alertIds+"&op=update","window","flushPage()",window);
					}else{
	...
	function btnDownLoad_onClick(){
		if(confirm("确认下载数据吗?")){
			...
			createTimerIfNull();
			window.location.href = "${contextPath}/filedownload/AStaffAcctTradDownloadAction.do?etlDateStart="+etlDateStart+"&etlDateEnd="+etlDateEnd+"&custNo="+custNo+
