...
<@CommonQueryMacro.page title="交易监测日报自查审核">
	<@CommonQueryMacro.CommonQuery id="AStaffAcctTradApprove" init="false" submitMode="allchange" navigate="false" >
...
<script language="JavaScript">
	...
	function initCallGetter_post() {
		//交易起始日期
		AStaffAcctTradApprove_interface_dataset.setValue("etlDateStart",sysTxdate);
		//交易结束日期
		AStaffAcctTradApprove_interface_dataset.setValue("etlDateEnd",sysTxdate);
	}
	...
	function btnSelect_onClickCheck(button){
		var record = AStaffAcctTradApprove_dataset.firstUnit;
		var updateCount = 0;
		var alertIds;
		while(record){
		...
		showWin("告警信息、客户信息和调查信息","/fpages/hf/form/ftl/AStaffAcctTradUpdate.ftl?alertIds="+alertIds+"&op=approve","window","flushPage()",window);
	}
	...
	function btnSave_onClickCheck(){
		var approveStatusChoose = AStaffAcctTradApprove_dataset.getValue("approveStatusChoose");
		var approveResultChoose = AStaffAcctTradApprove_dataset.getValue("approveResultChoose");
		var recordUpdTlrChoose = AStaffAcctTradApprove_dataset.getValue("recordUpdTlrChoose");
		...
		AStaffAcctTradApprove_dataset.setParameter("approveStatusChoose",approveStatusChoose);
		AStaffAcctTradApprove_dataset.setParameter("approveResultChoose",approveResultChoose);
	    AStaffAcctTradApprove_dataset.setParameter("recordUpdTlrChoose",recordUpdTlrChoose);
	...
	//刷新数据
	function flushPage(){
		AStaffAcctTradApprove_dataset.flushData(AStaffAcctTradApprove_dataset.pageIndex);
	}
</script>
