...
<script language="JavaScript">
	...
	function btnDownLoadP_onClick(){
		if(confirm("确认下载数据吗?")){
			...
			createTimerIfNull();
			window.location.href = encodeURI(encodeURI("${contextPath}/filedownload/CustSearchResultDownloadAction.do?qDateStart="+qDateStartP+
			"&qDateEnd="+qDateEndP+
			"&qTypeData="+qTypeDataP+
			"&qBatchNo"+qBatchNoP+
			"&tableName="+tableName));
		}
		return false;
	...
	function btnDownLoadL_onClick(){
		if(confirm("确认下载数据吗?")){
			...
			createTimerIfNull();
			window.location.href = encodeURI(encodeURI("${contextPath}/filedownload/CustSearchResultDownloadAction.do?qDateStart="+qDateStartL+
			"&qDateEnd="+qDateEndL+
			"&qTypeData="+qTypeDataL+
			"&qBatchNo"+qBatchNoL+
			"&tableName="+tableName));
		}
		return false;
	...
	function btnDownLoadC_onClick(){
		if(confirm("确认下载数据吗?")){
			...
			createTimerIfNull();
			window.location.href = encodeURI(encodeURI("${contextPath}/filedownload/CustSearchResultDownloadAction.do?qDateStart="+qDateStartC+
			"&qDateEnd="+qDateEndC+
			"&qTypeData="+qTypeDataC+
			"&qBatchNo"+qBatchNoC+
			"&tableName="+tableName));
		}
		return false;
