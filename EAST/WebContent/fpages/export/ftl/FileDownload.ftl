...
<script>
    ...
    function download(path){
        window.location.href = encodeURI(encodeURI("${contextPath}/filedownload/demoDownloadAction.do?path="+path));
    }
    
    //下载Excel
    function downloadExcel(path){
   		window.location.href = encodeURI(encodeURI("${contextPath}/filedownload/demoDownloadExcelAction.do?path="+path));
    }
</script>
