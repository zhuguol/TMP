...
<@CommonQueryMacro.page title="员工交易监测月报">
	<@CommonQueryMacro.CommonQuery id="AStaffAcctTradMS" init="false" submitMode="allchange" navigate="false" >
...
<script language="JavaScript">
	...
	function initCallGetter_post() {
		//交易起始日期
		AStaffAcctTradMS_interface_dataset.setValue("etlDateStart",sysTxdate);
		//交易结束日期
		AStaffAcctTradMS_interface_dataset.setValue("etlDateEnd",sysTxdate);
		//员工编号
		AStaffAcctTradMS_interface_dataset.setValue("staffNumbers","01");
	}
	...
	function btnSelect_onClickCheck(button){
		var record = AStaffAcctTradMS_dataset.firstUnit;
		var updateCount = 0;
		var alertIds;
		while(record){
			...
			if(staffNumber == ""){
				...
				if(approveStatus == "03"){
					...
					if(approveStatus != "00" && recordUpdTlr != ""){
			    		showWin("告警信息、客户信息、调查信息和质检信息","/fpages/hf/form/ftl/AStaffAcctTradMUpdate.ftl?alertIds="+alertIds+"&op=make","window","flushPage()",window);
					}else{
						alert("当前记录还未进行调查！");
						return false;
					}
	...
	function btnSubmit_onClickCheck(){
		var checkDateChoose = AStaffAcctTradMS_dataset.getValue("checkDateChoose");
		if(checkDateChoose.length == ''){
			alert("请选择初步调查日期！");
		   	return false;
		}
	    AStaffAcctTradMS_dataset.setParameter("checkDateChoose",checkDateChoose);
	...
	//刷新数据
	function flushPage(){
		AStaffAcctTradMS_dataset.flushData(AStaffAcctTradMS_dataset.pageIndex);
	}
</script>
